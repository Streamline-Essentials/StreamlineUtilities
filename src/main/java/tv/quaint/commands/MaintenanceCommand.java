...
public class MaintenanceCommand extends ModuleCommand {
    ...
    public MaintenanceCommand() {
        ...
        messageResultAdd = getCommandResource().getOrSetDefault("messages.result.add",
                "&eAdded &d%streamline_parse_%this_other%:::*/*streamline_user_formatted*/*% &eto &cMaintenance Mode whitelist&e.");
        messageResultRemove = getCommandResource().getOrSetDefault("messages.result.remove",
                "&eRemoved &d%streamline_parse_%this_other%:::*/*streamline_user_formatted*/*% &efrom &cMaintenance Mode whitelist&e.");
    }
...
//        }
        ...
        if (strings.length == 1) {
            ...
            try {
                ...
                if (bool) {
                    ModuleUtils.getOnlinePlayers().forEach((s, player) -> {
                        if (StreamlineUtilities.getMaintenanceConfig().getAllowedUUIDs().contains(player.getUuid()))
                            return;
